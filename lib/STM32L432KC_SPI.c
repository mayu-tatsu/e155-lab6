...
void initSPI(int br, int cpol, int cpha) {
  ...
  RCC->APB2ENR |= _VAL2FLD(RCC_APB2ENR_SPI1EN, 0b1);               // enable spi1 clk
  ...
  // SPI1->CR2 &= ~(SPI_CR2_FRXTH_Msk | SPI_CR2_SSOE_Msk);    // clear
  ...
  // SPI1->CR2 |= _VAL2FLD(SPI_CR2_SSOE, 0b1);         // Enable peripheral select mode  // maybe not needed
